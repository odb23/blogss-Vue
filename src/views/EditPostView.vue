...
import { uploadBlogPhoto, getDownloadURL, uploadBlogCoverPhoto, uploadBlogPost, updatePost } from "../services/post"
  ...
  if (blogTitle.value.length !== 0 && blogHTML.value.length !== 0) {
    if (file.value) {
      ...
      upload.on("state_changed",
        ...
        async () => {
          await updateBlogPostDocWithImage(upload)

        })
      return
    } else {
      loading.value = true
      await updateBlogPostDoc({
        blogId: routeId.value,
        blogHTML,
        blogTitle
      })
      loading.value = false
      $router.push({name: "ViewBlog", params: {
        blogid: routeId.value
      }})
      return
    }
...
async function updateBlogPostDoc(post) {
  try {
...
async function updateBlogPostDocWithImage(upload) {
  try {
    const coverPhotoURL = await getDownloadURL(upload.snapshot.ref);

    const blogId = await updateBlogPostDoc({
      blogId: routeId.value,
